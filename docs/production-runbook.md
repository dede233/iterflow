...
Web、API、PostgreSQL、Redis 默认仅绑定本机回环地址。仅在 **Web 绑定 loopback、前置 HTTPS 代理受信，且前置代理覆盖客户端传来的 `X-Real-IP`** 时设置 `TRUST_PROXY_HEADERS=true`；内置 Web Nginx 保留上游可信 `X-Real-IP`，无此头时使用直接连接地址。若 Web/API 对外直连，应关闭 `TRUST_PROXY_HEADERS`，避免伪造审计 IP。Nginx 的 52 MiB body 上限容纳 50 MiB 附件与 multipart 开销。生产 JSON 运行日志写 stdout，可用 `docker compose -f deploy/docker-compose.yml logs api` 查看；按 `request_id` 关联业务审计。日志不得导出密码、Token 或存储密钥。
...
S3 模式：先确认对象存储服务商的 Versioning、Snapshot、Replication 或 Provider Backup 恢复点，再在维护窗口执行 `bash deploy/scripts/backup-database.sh /absolute/new/database-backup-directory`。该脚本短暂停止 Web/API，仅导出 PostgreSQL、清单和校验和；清单明确 `storage_mode=s3`、`objects_included=false`，**不包含 S3 对象**。升级前验证对象恢复点与数据库备份时间一致。Local 模式也可使用此数据库单独备份命令，但它不替代包含 uploads 的完整备份。

## 恢复与回滚

Local Storage 恢复会覆盖当前数据库和 uploads，仅在维护窗口由授权运维执行。先保留现有环境额外备份，并核验目标备份文件、清单和校验和。运行 `CONFIRM_RESTORE=YES bash deploy/scripts/restore.sh /absolute/backup-directory`。脚本停止入口和迁移/初始化容器，连接 PostgreSQL 维护库强制断开应用连接，**删除并重建 iterflow 数据库**，再恢复 dump 与附件、读取备份的 `alembic_version`，运行 migrate/seed 并检查 `/ready`。因此备份后新增的表、索引等对象不会残留。恢复完成还必须人工验证登录、反馈查询、业务附件下载及其 SHA256。任何破坏性步骤失败时 Web/API 保持停止，不要向用户开放不完整数据。S3 模式的数据库恢复须配合服务商对象恢复点单独制定操作计划；此 Local 恢复脚本拒绝 S3 模式。
