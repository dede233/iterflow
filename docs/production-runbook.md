...
要求 Docker Engine、Compose v2、可靠的 PostgreSQL/Redis 卷、备份存储与前置 HTTPS 代理。复制根目录 `.env.example` 为 `.env`，权限设为仅部署用户可读。填写 `POSTGRES_PASSWORD`、Docker 网络内的 `DATABASE_URL`、`REDIS_URL`、至少 32 字节随机 `JWT_SECRET`、`INIT_ADMIN_USERNAME` 和 20 位以上随机 `INIT_ADMIN_PASSWORD`。生产设置 `APP_ENV=production`、实际 `ALLOWED_HOSTS`、`TRUST_PROXY_HEADERS=true`、`ENABLE_API_DOCS=false`、`BUILD_SHA` 为部署提交 SHA；同源部署可令 `CORS_ORIGINS` 为空。不可配置通配 Host 或带凭据的通配 CORS。配置 `WEB_BIND_HOST=127.0.0.1`，由 HTTPS 反向代理向外提供服务；**不得直接公开纯 HTTP 8080 端口**。本阶段不签发 TLS 证书。
